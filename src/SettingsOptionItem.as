...
class SettingsOptionItem extends MovieClip
{
	...
	function get value(): Number
	{
		...
		switch (iMovieType) {
			case 0:
				iFrameValue = ScrollBar_mc.position;
				break;
	...
	function set value(afNewValue: Number): Void
	{
		switch (iMovieType) {
			case 0:
				...
				ScrollBar_mc.position = afNewValue;
				bSendChangeEvent = true;
				break;
			...
			switch (iMovieType) {
				case 0:
					if (details.navEquivalent == NavigationCode.LEFT) {
						ScrollBar_mc.position -= fInterval;
						bhandledInput = true;
				...
				case 2:
					if (details.navEquivalent == NavigationCode.ENTER && details.code != Key.SPACE) {
						ToggleCheckbox();
						bhandledInput = true;
					}
					break;
	...
	function SetSliderProperties(minimum: Number, maximum: Number, interval: Number): Void
	{
		bSendChangeEvent = false;
		var pageSize = (maximum - minimum) / 30;
		ScrollBar_mc.setScrollProperties(pageSize, minimum, maximum);
