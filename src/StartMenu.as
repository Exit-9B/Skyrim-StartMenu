...
class StartMenu extends MovieClip
{
	...
	function StartMenu()
	{
		...
		_root.ReleaseCodeObject = Proxy.create(this, this.ReleaseCodeObject);
		_root.onCodeObjectInit = Proxy.create(this, this.onCodeObjectInit);
		...
		_loc5_.onLoadInit = Proxy.create(this, this.OnLoginLoadInit);
	...
	{
		mc._visible = false;
		mc.onEnterFrame = Proxy.create(this, this.OnLoginLoadInitFinished, mc);
	}
	...
	function OnLoginLoadInitFinished(mc)
	{
		...
		if (this._LoginHolder_mc.LoginMenu_mc.Constructed)
		{
			...
			this._LoginMenu.addEventListener(BethesdaNetLogin.LOGIN_CANCELED, Proxy.create(this, this.onLoginCanceled));
			this._LoginMenu.addEventListener(BethesdaNetLogin.LOGIN_ERROR, Proxy.create(this, this.onLoginError));
	...
	function InitExtensions()
	{
		...
		this.SaveLoadListHolder.addEventListener("OnSaveLoadPanelBackClicked", Proxy.create(this, this.OnSaveLoadPanelBackClicked));
		...
		this.CharacterSelectionHint.addEventListener("OnMousePressCharacterChange", Proxy.create(this, this.OnMousePressCharacterChange));
	}
	...
	function setupMainMenu()
	{
		...
		this._UserCanAccessCreationClub = arguments[_loc6_];
	...
	function handleInput(details, pathToFocus)
	{
		...
		{
			if (GlobalFunc.IsKeyPressed(details) && this.ShouldProcessInputs)
			{
				...
				else if ((details.navEquivalent == NavigationCode.GAMEPAD_X || details.code == 88) && this.DeleteSaveButton._visible && this.DeleteSaveButton._alpha == StartMenu.ALPHA_AVAILABLE)
				{
					if (this.IsPlatformSony())
					{
				...
				else if ((details.navEquivalent == NavigationCode.GAMEPAD_Y || details.code == 84) && this.strCurrentState == StartMenu.SAVE_LOAD_STATE && !this.SaveLoadListHolder.isSaving)
				{
					GameDelegate.call("PlaySound", ["UIMenuCancel"]);
					this.EndState();
				}
				else if ((details.navEquivalent == NavigationCode.GAMEPAD_X || details.code == 88) && this.currentState == StartMenu.MAIN_STATE)
				{
					GameDelegate.call("Sky10DLCPressed", []);
				}
				else if (details.navEquivalent == NavigationCode.GAMEPAD_Y && this.currentState == StartMenu.DLC_STATE && this.MarketplaceButton._visible && this.MarketplaceButton._alpha == StartMenu.ALPHA_AVAILABLE)
				{
				...
				else if (details.navEquivalent == NavigationCode.GAMEPAD_Y && this.currentState == StartMenu.MAIN_STATE && this.ChangeUserButton._visible)
				{
	...
	{
		GameDelegate.call("PopulateCharacterList", [this.SaveLoadListHolder.List_mc.entryList, this.SaveLoadListHolder.batchSize]);
	...
	function SetPlatform(aiPlatform, abPS3Switch)
	{
		...
		if (aiPlatform == StartMenu.PLATFORM_PC_KBMOUSE)
		{
			...
			this.DeleteSaveButton.onPress = Proxy.create(this, this.onMouseButtonDeleteSaveClick);
			this.DeleteSaveButton.addEventListener("rollOver", Proxy.create(this, this.onMouseButtonDeleteRollOver));
		}
		else if (aiPlatform == StartMenu.PLATFORM_PC_GAMEPAD && this.DeleteSaveButton == this.DeleteMouseButton)
		{
			...
			this.DeleteMouseButton.removeEventListeners("rollOver", Proxy.create(this, this.onMouseButtonDeleteRollOver));
		}
		else
		{
	...
	function onFadeOutCompletion()
	{
		if (this.strFadeOutCallback != undefined && this.strFadeOutCallback.length > 0)
		{
			...
			{
				GameDelegate.call(this.strFadeOutCallback, this.fadeOutParams);
			}
			else
			{
				GameDelegate.call(this.strFadeOutCallback, []);
			}
		}
	}
	...
	function ChangeStateFocus(strNewState)
	{
		switch (strNewState)
		{
			case StartMenu.MAIN_STATE:
				FocusHandler.instance.setFocus(this.MainList, 0);
				break;
			case StartMenu.CHARACTER_SELECTION_STATE:
			case StartMenu.SAVE_LOAD_STATE:
				FocusHandler.instance.setFocus(this.SaveLoadListHolder.List_mc, 0);
				this.SaveLoadListHolder.List_mc.disableSelection = false;
				break;
			case StartMenu.DLC_STATE:
				this.iLoadDLCListTimerID = setInterval(this, "DoLoadDLCList", 500);
				FocusHandler.instance.setFocus(this.DLCList_mc, 0);
				break;
			case StartMenu.MAIN_CONFIRM_STATE:
			case StartMenu.SAVE_LOAD_CONFIRM_STATE:
			case StartMenu.DELETE_SAVE_CONFIRM_STATE:
			...
			case StartMenu.MARKETPLACE_CONFIRM_STATE:
				FocusHandler.instance.setFocus(this.ButtonRect, 0);
			default:
				return;
		}
	}
	...
	function DoLoadDLCList()
	{
		...
		GameDelegate.call("LoadDLC", [this.DLCList_mc.entryList], this, "UpdateDLCPanel");
	}
